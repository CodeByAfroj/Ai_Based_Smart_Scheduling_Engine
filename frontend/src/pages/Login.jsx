...
export default function Login() {
  ...
  const handleAuthSuccess = async (token) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMessage(`Backend Authentication Failed: ${res.status} ${res.statusText}`);
      }
    } catch (err) {
      console.error('Login failed', err);
      setErrorMessage(`Connection Error: Unable to reach backend server (${err.message})`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleWebGoogleLogin = useGoogleLogin({
    ...
    onError: errorResponse => {
      ...
      setErrorMessage(`Google Sign-In Error: ${errorResponse.error_description || errorResponse.error || 'Sign in popup closed or cancelled'}`);
      setIsLoading(false);
    },
  });
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 bg-[var(--bg-app)]">
        <div className="w-full max-w-md flex flex-col">
          ...
          {errorMessage && (
            <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-sm flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-red-400 mt-1">Check console logs or verify Google Cloud Console origins & redirect URIs.</p>
              </div>
            </div>
          )}
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-panel)] hover:bg-[var(--bg-hover)] transition-colors mb-6 font-semibold text-[var(--text-main)] shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <Loader2 className="w-5 h-5 animate-spin text-[var(--accent-base)]" />
            ) : (
